    ...
    def should_delete(self, index):
        ...
        if not re.search(self.idx_regex, index["index"]):
            return False, "index '{}' name '{}' did not match pattern '{}'".format(index["index"],
                                                                                   idx_name,
                                                                                   self.idx_regex)
        if '-' in self.idx_format:
        # date format with hours
            try:
                idx_date = datetime.datetime.strptime(idx_date_str, self.idx_format)
            except ValueError:
                return False, "index '{}' name '{}' could not be parsed with format '{}'".format(index["index"],
                                                                                               idx_name,
                                                                                               self.idx_format)
        else:
        # date format without hours
            idx_date_str = idx_date_str[:10]
            try:
                idx_date = datetime.datetime.strptime(idx_date_str, self.idx_format)
            except ValueError:
                return False, "index '{}' name '{}' could not be parsed with format '{}'".format(index["index"],
        ...
        if re.search(self.skip_idx_regex, index["index"]):
            return False, "index '{}' name '{}' matches skip pattern '{}'".format(index["index"],
                                                                               idx_name,
                                                                               self.skip_idx_regex)
